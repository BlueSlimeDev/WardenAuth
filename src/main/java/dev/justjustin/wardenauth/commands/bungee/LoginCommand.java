...
@Command(
        description = "Login Command of the plugin",
        shortDescription = "Login Command"
)
public class LoginCommand implements SlimeCommand {
    ...
    public <T> LoginCommand(WardenAuth<T> plugin) {
        this.plugin = (WardenAuth<Plugin>) plugin;

        Control control = plugin.getLoader().getFiles().getControl(SlimeFile.COMMANDS);

        this.command = control.getString("commands.login.command", "login");

        this.aliases.addAll(
                control.getStringList("commands.login.aliases")
        );
